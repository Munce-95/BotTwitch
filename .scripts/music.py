...
class MusicManager:
    ...
    def save_to_cache(self, title, artist, track_uri):
        ...
        print(f"📖 Apprentissage : {title} - {artist}")
        ...
        while self.running:
            try:
                ...
                if e.http_status == 429:
                    ...
                    print(f"⚠️ [RATE LIMIT] Pause forcée : {wait}s.")
                    time.sleep(wait)
                else:
                    time.sleep(30)
            except:
    ...
    def handle_sr(self, user, query, tags, send_msg_func):
        try:
            ...
            if e.http_status == 429:
                send_msg_func("⚠️ Spotify est fatigué, réessaie plus tard.")
        except: pass
